        ...
        for node_id in evaluation_order:
            ...
            if node_type == NodeType.constant:
                ...
                if len(input_edges) < 2:
                    raise HTTPException(
                        status_code=400, 
                        detail=f"Operation node {node_id} has insufficient inputs"
                    )
                ...
                for source, target in input_edges:
                    if source not in computed_values:
                        raise HTTPException(
                            status_code=400, 
                            detail=f"Input node {source} not yet computed"
                        )
                ...
                if condition_result:
                    computed_values[node_id] = node_data["data"]["true_value"]
                else:
                    computed_values[node_id] = node_data["data"]["false_value"]
                
                if computed_values[node_id] is None:
                    input_edges = list(G.in_edges(node_id))
                    if input_edges:
                        source = input_edges[0][0]
                        if source in computed_values:
                            computed_values[node_id] = computed_values[source]
                        else:
